...
<html>
<head>
	<link rel="stylesheet"  href="style_dlc_site_gen.css">
...
<body class="body">
	...
	<div class="game_site_content_and_description_and_requirement_container">
		<div class="game_site_left_container">
			<p class="game_site_headline">ABOUT THIS CONTENT</p> <hr style="background-color: white;width:60%;text-align:left;margin-left:0">
...
<script>
	...
	const dlc_name = urlParams.get('dlc_name')
	console.log(game_name);
	console.log(dlc_name);
	document.getElementById("title").innerHTML=game_name+" : "+dlc_name;
	document.getElementById("Tab_header_logo").innerHTML=game_name+" : "+dlc_name;
	...
	////Start minor image////
    $.post("get_column_dlc_image_specific_game.php",
    {
        current_game_name:game_name,current_dlc_name:dlc_name
    })
    .done(function(data)
    {
         try
				{
					var dlc_minor_image = JSON.parse(data);
				}
				catch
				{
					var dlc_minor_image = "";
				}
				console.log(dlc_minor_image[0]["game_names"]);
				console.log(dlc_minor_image[0]["dlc_image_name"]); 

				var template_four_minor_image = "";
				var template_big_image = "";
				var highest_number_minor_images_show = 4;
				template_big_image = '<img src="../game_list\\'+game_name+'\\dlc_list\\'+dlc_minor_image[0]['dlc_names']+'\\images\\'+dlc_minor_image[0]["dlc_image_name"]+' "class="game_site_show_big_image" id="big_image">';
				$("#game_site_show_big_image").append(template_big_image);
				for (var i = 0; i < dlc_minor_image.length; i++) 
				{
					if (i < highest_number_minor_images_show) 
					{
						template_four_minor_image = template_four_minor_image +'<img src="../game_list\\'+game_name+'\\dlc_list\\'+dlc_minor_image[i]['dlc_names']+'\\images\\'+dlc_minor_image[i]["dlc_image_name"]+'" class="game_site_show_small_image" name="minor_image" id="minor_image_'+i+'"; onclick="show_small_image_to_big_image(\''+dlc_minor_image[i]['dlc_image_name']+'\')"/>';
					}
					else
					{
						template_four_minor_image = template_four_minor_image +'<img src="../game_list\\'+game_name+'\\dlc_list\\'+dlc_minor_image[i]['dlc_names']+'\\images\\'+dlc_minor_image[i]["dlc_image_name"]+'"  class="game_site_show_small_image_hidden" name="minor_image" id="minor_image_'+i+'"; onclick="show_small_image_to_big_image(\''+dlc_minor_image[i]['dlc_image_name']+'\')"/>';
					}
				}
	...
	////Start game detail////
    $.post("get_column_dlc_detail_specific_game.php",
    {
        current_dlc_name:dlc_name
    })
    .done(function(data)
    {
        try
			{
				var dlc_detail = JSON.parse(data);
			}
			catch
			{
				var dlc_detail = "";
		...
		var template_dlc_list = '<p style="color:white;"><span style="color:#3366ff">\
								Release date </span> : '+dlc_detail[0]['dlc_release_date']+'</p><br>\
								<p style="color:white;"><span style="color:#3366ff">Developers </span> : '+dlc_detail[0]['developers']+'</p>\
								<p style="color:white;"><span style="color:#3366ff">\
								Publishers </span> : '+dlc_detail[0]['publishers']+'</p><br>\
		...
		console.log(dlc_detail);
		if (dlc_detail.length<5) 
		{
			var highest_number_of_tags_show = dlc_detail.length;
		}
		else
		{
		...
		{
			template_show_tags = template_show_tags+'<div class="small_tags_detail">'+dlc_detail[i]["tag_name"]+'</div>';
		}
		var all_tags_this_game = [];
		for (var i = 0; i < dlc_detail.length; i++) 
		{
			all_tags_this_game.push(dlc_detail[i]["tag_name"]);
		}
		...
		var all_dlc_detail =  template_dlc_list+template_formatted_tags;
		$("#game_site_show_game_detail_container").append(all_dlc_detail);
    });
	////End game detail////

	///Start game price////
    $.post("get_column_dlc_price_specific_game.php",
    {
        current_game_name:game_name,current_dlc_name:dlc_name
    })
    .done(function(data)
    {
        try
			...
			//console.log(price);
			var template_show_tags = '<div class="current_state_price">&#x0E3F '+price[0]['dlc_price']+'</div>'
			$("#add_cart_btn_container").append(template_show_tags);
    });
	////End game price////

	///Start description////
    $.post("get_column_dlc_description_specific_game.php",
    {
        current_game_name:game_name,current_dlc_name:dlc_name
    })
    .done(function(data)
    {
        try
			...
			template_description = '<iframe src=\"../game_list/'+description[0]["game_names"]+'/dlc_list/'+description[0]["dlc_names"]+'/description/'+description[0]["description_name"]+'\" class="game_site_description"></iframe>';
	...
	function show_small_image_to_big_image(minor_image)
	{
		var current_big_image = document.getElementById("big_image");
  		...
  		template = '<img src="../game_list\\'+game_name+'\\dlc_list\\'+dlc_name+'\\images\\'+minor_image+'" class="game_site_show_big_image" id="big_image">'
